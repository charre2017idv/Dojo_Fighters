...
public class PLAYER_XBOX : MonoBehaviour
{
    ...
    public PLAYER_XBOX ohterPlayer;
    ...
    void movement()
    {
        ...
        if (!b_isAttack)
        {
            if (Input.GetAxis("EjeX_izq") > 0)
            {
                m_move.moveRigth();
            }
            else if (Input.GetAxis("EjeX_izq") < 0)
            {
                m_move.moveleft();
            }
            else
    ...
    void attack()
    {
        if (Input.GetButtonDown("BotonA") && !m_attack.bIsAttacking && !m_dash.isDash)
        {
        ...
        else if (Input.GetButtonDown("BotonB") && !m_attack.bIsAttacking && !m_dash.isDash)
        {
            if (m_attack.bIsAuraActivated)
                m_sound.PlaySound(2);
            else
        ...
        else if (Input.GetButtonDown("BotonX") && !m_attack.bIsAttacking && !m_dash.isDash)
        {
            m_attack.ActivateAttack();
        }
        else if (Input.GetButtonDown("BotonY") && m_attack.bIsAuraActivated)
        {
            if (m_attack.bIsAuraActivated)
                m_sound.PlaySound(2);
